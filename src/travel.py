...
class TravelTime(object):
    ...
    def velocity(self, dep):
        ...
        i_dep = int(2 * dep)
        dep -= i_dep * 0.5
        return (self._velocity[i_dep] * (0.5 - dep) + self._velocity[i_dep + 1] * dep) * 2
    ...
    def __init__(self):
        ...
        with open(os.path.join(self.jma2001, 'take_off_angle')) as f:
            for line in f.readlines():
                ...
                t[x.index(dep), y.index(dis)] = (math.asin(math.sin(ang) / self.velocity(float(dep)) * self.velocity(0)))
