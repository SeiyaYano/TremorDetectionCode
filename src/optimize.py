...
def _cross_correlation_grad(x, init_pos, cc, ind, station, weight, station_i):
    ...
    grad = g * w * (dt[j_s, :] - dt[i_s, :]).astype(np.float64)
...
def _optimize(x, cc, ind, station, weight):
    ...
    #     _cross_correlation_obj, [0, 0, 0], method='L-BFGS-B',
    #     jac=_cross_correlation_grad, args=args,
    #     bounds=((-const.max_dist_from_grid, const.max_dist_from_grid), 
    #             (-const.max_dist_from_grid, const.max_dist_from_grid),
...
class Optimizer(object):
    def __init__(self, x, cc, ind, station, env_or_wav, channel_path):
        self.x = x 
        self.station = station
        self.env_or_wav = env_or_wav
        self.channel_path = channel_path
        self.cc = cc
        self.ind = np.asarray(ind, dtype=np.int32)
        self.n_c = max(self.cc.i_c[ind].max(), self.cc.j_c[ind].max()) + 1

        self.components = set(zip(self.cc.i_s[self.ind], self.cc.i_c[self.ind])) | set(zip(self.cc.j_s[self.ind], self.cc.j_c[self.ind]))
    ...
    def get_template(self, waveform, tp, tmin):
        ...
        wave1, wave2, factor1, factor2 = self._shift_waveform(waveform, tp, tmin)
        weight = cnt[self.components[:, 1]] * self.weight[self.components[:, 1]]
    ...
    def _update_weight(self, template, tp, tmin):
        env1, env2, factor1, factor2 = self._shift_waveform(self.env_or_wav, tp, tmin)
    ...
    def _cc_template(self, template, tp, tmin):
        cc_t = np.zeros(len(self.env_or_wav), dtype=np.float32)
        env1, env2, factor1, factor2 = self._shift_waveform(self.env_or_wav, tp, tmin)
    ...
    def __call__(self):
        while True:
            opt_result = _optimize(self.x, self.cc, self.ind, self.station, self.weight)
            self.x = opt_result.x
            
            if not opt_result.success or self.x[2] < 0.1 or const.max_source_depth_search - 0.1 < self.x[2]:
                return None
            ...
            template = self.get_template(self.env_or_wav, tp, tmin)
            cc_tw = self._cc_template(template, tp, tmin)
            cc = self.cc(self.ind,tp[self.cc.j_s[self.ind]] - tp[self.cc.i_s[self.ind]])
