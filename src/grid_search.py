...
class GridSearch(object):
    def __init__(self, station):
        ...
        self.lamin = math.floor((self.lamin - 2) * 5)
        self.lamax = math.ceil((self.lamax + 2) * 5)
        self.lomin = math.floor((self.lomin - 2) * 5)
        self.lomax = math.ceil((self.lomax + 2) * 5)

        self.grid_tp = {}
        for la in range(self.lamin, self.lamax + 1):
            for lo in range(self.lomin, self.lomax + 1):

                dis = distance(la / 5.0, lo / 5.0, station[:, 0], station[:, 1])
    ...
    def grid(self, sac, cc):
        ...
        for la, lo in pos:
            ...
            grid[la, lo] = optimize.cross_correlation(cc, ind, self.grid_tp[(la, lo)], weight)
        return grid, pos
